...
genGraph <- function(file){
  ...
  legend("topleft", legend=c("total", "nussinov", "traceback"),
         col=c(2,3,4), lty=1,lwd=2,cex=1.8,bty = "n")
  detach(data)
  print(summary(lm1))
  print(summary(lm2))
  print(summary(lm3))
}
genGraph("./testcase/test16-512.txt")
...
genGraph2 <- function(file){
  ...
  plot(length, total, pch=20,xlab="length",ylab="time",
       main="length vs total time O(n^3)")
  lines(len1, time1, lty=1, lwd=2, col="blue") 
  ...
  plot(length, total, pch=20,xlab="length",ylab="time",
       main="length vs nussinov time O(n^3)")
  lines(len1, time2, lty=1, lwd=2, col="blue") 
...
genGraph2("./testcase/test4-12power.txt")
